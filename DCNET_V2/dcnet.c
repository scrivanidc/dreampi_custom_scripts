...
int configureTty(int fd, int local)
{
	...
	int baud = atoi(ttyBaud);
	speed_t rate = getBaudRate(baud);
	cfsetispeed(&tbuf, rate);
	cfsetospeed(&tbuf, rate);
...
speed_t getBaudRate(int baud) {
    switch (baud) {
        ...
        default:
            fprintf(stderr, "Baud rate %d not supported, using 57600\n", baud);
            return B57600;
    }
}
...
int main(int argc, char *argv[])
{
	int forceHost = 0;
	int opt;
	while ((opt = getopt(argc, argv, "t:b:h:p:")) != -1)
	{
		switch (opt)
		{
		case 't':
